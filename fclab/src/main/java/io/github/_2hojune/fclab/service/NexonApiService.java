...
public class NexonApiService {
    ...
    // 1. 시즌 정보를 담아둘 메모리 캐시 (서버가 켜질 때 한 번만 채워짐)
    ...
    // 💡 2. 서버 실행 시 딱 한 번 실행되는 초기화 메서드 (넥슨 시즌 데이터 긁어오기)
    ...
    public void initSeasonCache() {
        ...
        try {
            ...
            for (JsonNode season : sesonArray) {
                int seasonId = season.get("seasonId").asInt();
                String className = season.get("className").asText();
                seasonCache.put(seasonId, className);
            }
            System.out.println("✅ FC Lab: 시즌 메타데이터 캐싱 완료! (" + seasonCache.size() + "개 로드)");
        ...
        try {
            ...
            for (JsonNode playerNode : rootArray) {
                if (playerNode.get("name").asText().contains(name)) {
                    ...
                    // 💡 4. 우리 캐시에서 시즌 이름 찾기 (만약 없으면 "Unknown" 반환)
                    String seasonName = seasonCache.getOrDefault(seasonId, "Unknown");

                    // 💡 5. JSON 객체(playerNode)에 seasonName 데이터를 끼워넣기 위해 ObjectNode로 변환
